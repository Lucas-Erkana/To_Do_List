...
class Formatter {
  ...
  formatDateTimeFromString(dt, fmt) {
    ...
    const meridiem = () => (knownEnglish ? meridiemForDateTime(dt) : string({ hour: 'numeric', hourCycle: 'h12' }, 'dayperiod'));
    const month = (length, standalone) => (knownEnglish ? monthForDateTime(dt, length) : string(standalone ? { month: length } : { month: length, day: 'numeric' }, 'month'));
    const weekday = (length, standalone) => (knownEnglish ? weekdayForDateTime(dt, length) : string(standalone ? { weekday: length } : { weekday: length, month: 'long', day: 'numeric' }, 'weekday'));
...
function explainFromTokens(locale, input, format) {
  ...
  const [result, zone, specificOffset] = matches ? dateTimeFromMatches(matches) : [null, null, undefined];
...
class DateTime {
  ...
  static fromObject(obj, opts = {}) {
    ...
    const tsNow = Settings.now();
    const offsetProvis = !isUndefined(opts.specificOffset) ? opts.specificOffset : zoneToUse.offset(tsNow);
   ...
   */
  toISO({
    format = 'extended', suppressSeconds = false, suppressMilliseconds = false, includeOffset = true,
  } = {}) {
    if (!this.isValid) {
      return null;
    }
   ...
   */
  toISOTime({
    suppressMilliseconds = false, suppressSeconds = false, includeOffset = true, includePrefix = false, format = 'extended',
  } = {}) {
    if (!this.isValid) {
      return null;
    }
